...
st.set_page_config(page_title="GEXTIA FACTORY PRO", layout="wide", page_icon="🔘")
...
st.markdown("""
    ...
    h1, h2, h3 { color: #1A1A1A; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; letter-spacing: -0.5px; }
    .stMarkdown { font-size: 14px; }
    
    /* Sidebar */
    [data-testid="stSidebar"] { background-color: #F2F2F2; border-right: 1px solid #E0E0E0; }
    </style>
...
with st.sidebar:
    ...
    if not st.session_state.mesa.empty or not st.session_state.bom.empty:
        st.download_button(
            label="EXPORTAR SESIÓN (.PKT)",
            data=guardar_progreso(),
            file_name=f"Sesion_{datetime.now().strftime('%d%m_%H%M')}.pkt",
            use_container_width=True
        )
    
    st.write("---")
    archivo_subido = st.file_uploader("RESTAURAR SESIÓN", type=["pkt"])
    if archivo_subido:
        if st.button("CONFIRMAR RESTAURACIÓN", use_container_width=True):
            cargar_progreso(archivo_subido.read())
            st.rerun()

    if st.button("RESET DE DATOS", use_container_width=True):
...
# --- 5. TABS ---
t1, t2, t3, t4 = st.tabs(["MESA DE CORTE", "ASIGNACIÓN", "GEXTIA IMPORT", "PURCHASE LIST"])
...
with t1:
    ...
    if df_prendas is not None:
        ...
        with c_sel: seleccion_refs = st.multiselect("Seleccionar Referencias:", sorted(df_prendas['Referencia'].unique()))
        with c_btn:
            st.write(" ") # Espaciador
    ...
    if not st.session_state.mesa.empty:
        ...
        with c_all:
            if st.checkbox("Seleccionar todo", key="master_sel") != st.session_state.get('p_sel', False):
                st.session_state.mesa['Sel'] = st.session_state.master_sel
                st.session_state['p_sel'] = st.session_state.master_sel
                st.rerun()
        with c_talla:
            t_target = st.selectbox("Filtro Talla:", ["Todas"] + sorted(st.session_state.mesa['Talla'].unique().tolist()))
        with c_ops:
            ...
            if b4.button("Eliminar"): st.session_state.mesa = st.session_state.mesa[~mask].reset_index(drop=True); st.rerun()
        ...
        for idx, row in st.session_state.mesa.iterrows():
            ...
            nv = f4.number_input("n", min_value=0, value=int(row['Cant. a fabricar']), key=f"v_{idx}", label_visibility="collapsed", step=1)
...
with t2:
    if not st.session_state.mesa.empty:
        ...
        with c_c: 
            cons_inj = st.number_input("Consumo por prenda:", min_value=0.0, value=1.0, format="%.3f")
        
        st.write("---")
        f1, f2, f3 = st.columns(3)
        with f1: r_ts = st.multiselect("Filtrar por Ref:", sorted(st.session_state.mesa['Referencia'].unique()))
        with f2:
            d_t = st.session_state.mesa if not r_ts else st.session_state.mesa[st.session_state.mesa['Referencia'].isin(r_ts)]
            c_ts = st.multiselect("Filtrar por Color:", sorted(d_t['Color'].unique()))
        with f3:
            d_t2 = d_t if not c_ts else d_t[d_t['Color'].isin(c_ts)]
            t_ts = st.multiselect("Filtrar por Talla:", sorted(d_t2['Talla'].unique()))
            
        final_df = d_t2 if not t_ts else d_t2[d_t2['Talla'].isin(t_ts)]
        st.info(f"Variantes seleccionadas: {len(final_df)}")
        
        col_b1, col_b2 = st.columns([4, 1])
        with col_b1:
            if st.button("PROCESAR ASIGNACIÓN", use_container_width=True):
...
with t3:
    if not st.session_state.bom.empty:
        ...
        st.download_button("DESCARGAR EXCEL GEXTIA", out.getvalue(), f"Gextia_BOM_{datetime.now().strftime('%H%M')}.xlsx")

# --- TAB 4: COMPRAS ---
with t4:
    if not st.session_state.bom.empty:
        st.subheader("Resumen de Necesidades de Compra")
        ...
        st.download_button("DESCARGAR LISTA DE COMPRA", out_c.getvalue(), "Lista_Compra.xlsx")
